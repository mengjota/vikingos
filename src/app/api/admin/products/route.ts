...
export async function GET(req: NextRequest) {
  ...
  const bid = await getOwnerBarbershop(email);
  if (!bid) return NextResponse.json({ error: "No autorizado" }, { status: 401 });

  const rows = await sql`
    SELECT id, name, description, price, price_display, volume, category, featured, active
    FROM products WHERE barbershop_id = ${bid} ORDER BY category, id ASC
  `;
...
export async function POST(req: NextRequest) {
  ...
  const [row] = await sql`
    ...
    VALUES (${bid}, ${name}, ${description ?? ""}, ${priceNum}, ${priceDisplay}, ${volume ?? ""}, ${category ?? "General"}, ${featured ?? false})
    RETURNING id, name, description, price, price_display, volume, category, featured, active
  `;
...
export async function PUT(req: NextRequest) {
  ...
  const [row] = await sql`
    UPDATE products SET
      ...
      price         = COALESCE(${priceNum ?? null}, price),
      price_display = COALESCE(${priceDisplay ?? null}, price_display),
      volume        = COALESCE(${volume ?? null}, volume),
      category      = COALESCE(${category ?? null}, category),
      featured      = COALESCE(${featured ?? null}, featured),
      active        = COALESCE(${active ?? null}, active)
    WHERE id = ${id} AND barbershop_id = ${bid}
    RETURNING id, name, description, price, price_display, volume, category, featured, active
  `;
