...
export default function AdminConfiguracion() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/mi-barberia", { headers: { "x-caller-email": s.email } })
      .then(r => r.json())
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    localStorage.setItem("inv_session", JSON.stringify({ ...s, barbershopName: json.name }));
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto", padding: "48px 24px" }}>
        ...
        {loading ? (
          ...
          <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "18px" }}>
                <div>
                  ...
                  <input required value={form.name} onChange={e => set("name", e.target.value)}
                    placeholder="Ej: Barbería del Norte" maxLength={60} style={S.input} />
                </div>
                <div>
                  <label style={S.label}>Teléfono de contacto</label>
                  <input value={form.phone} onChange={e => set("phone", e.target.value)}
                    placeholder="Ej: +34 612 345 678" maxLength={30} style={S.input} />
                </div>
                <div style={{ gridColumn: "1 / -1" }}>
                  <label style={S.label}>Dirección</label>
                  <input value={form.address} onChange={e => set("address", e.target.value)}
                    placeholder="Ej: Carrer de Mallorca 123, Barcelona" maxLength={120} style={S.input} />
                </div>
                <div style={{ gridColumn: "1 / -1" }}>
                  <label style={S.label}>Descripción breve</label>
                  <textarea value={form.description} onChange={e => set("description", e.target.value)}
                    placeholder="Una descripción de tu barbería para los clientes..." maxLength={300} rows={3}
            ...
            {msg && (
              <p style={{ color: msg.type === "ok" ? "#4ade80" : "#f87171", fontSize: "0.78rem", letterSpacing: "0.08em" }}>
            ...
            <button type="submit" disabled={saving}
              style={{
                ...
                background: saving ? "rgba(92,58,30,0.3)" : "linear-gradient(135deg,#a06010,#c8921a,#f0c040,#c8921a,#a06010)",
                border: "none", cursor: saving ? "not-allowed" : "pointer",
                fontFamily: "var(--font-barlow)", fontSize: "0.75rem", fontWeight: 800,
                letterSpacing: "0.4em", textTransform: "uppercase",
                color: saving ? "rgba(184,168,138,0.3)" : "#080604",
                boxShadow: saving ? "none" : "0 0 25px rgba(200,146,26,0.35)",
              }}>
